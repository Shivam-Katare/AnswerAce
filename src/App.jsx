...
function App() {
  ...
  useEffect(() => {
    const storedApiKey = window.localStorage.getItem('openaiApiKey');
    if (storedApiKey) {
      setApiKey(storedApiKey);
    }
  }, []);

  // Update the API key in local storage and state on change
  function handleApiKeyChange(e) {
    const newApiKey = e.target.value;
    setApiKey(newApiKey);
    window.localStorage.setItem('openaiApiKey', newApiKey);
  }
  ...
  const doStuff = async () => {
    ...
    try {
      ...
      setResult("Error: " + "Make sure you entered your OpenAI API key, and it must be correct!");
    } finally {
      setIsLoading(false);
    }
  };
